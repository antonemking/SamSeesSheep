"""3-way held-out benchmark: v0.4 vs v0.5 vs v0.7 on IMG_3651.

Outputs into ./artifacts:
  v0.7-on-IMG_3651.mp4                    Ultralytics-annotated inference
  v0.4-vs-v0.5-vs-v0.7-IMG_3651.mp4      3-up side-by-side
  v0.4-vs-v0.5-vs-v0.7-IMG_3651.png      4-frame stills grid
  ear_angle_lines-v04v05v07-IMG_3651.png  ear angle σ chart
  bench_report-IMG_3651-v04v05v07.json    headline numbers
"""
...
V07 = ROOT / "weights" / "sheep-pose-v0.7-yolo26n.pt"

V04_BGR = (255, 0, 255)    # magenta
V05_BGR = (255, 165, 0)    # blue-ish
V07_BGR = (0, 200, 80)     # green

V04_RGB = tuple(c / 255 for c in reversed(V04_BGR))
V05_RGB = tuple(c / 255 for c in reversed(V05_BGR))
V07_RGB = tuple(c / 255 for c in reversed(V07_BGR))
...
def predict_clip(model_path: Path, label: str):
    ...
    cache = CACHE_DIR / f"{model_path.stem}__{CLIP.stem}.pkl"
    ...
    target_kpts_per_frame, all_kpts_per_frame = [], []
...
def make_ear_angle_chart(targets: dict, fps: float, out_path: Path):
    ...
    n = b - a
    t = np.arange(n) / fps
    colors = {"v0.4": V04_RGB, "v0.5": V05_RGB, "v0.7": V07_RGB}
    ...
    fig.suptitle(
        "Ear angle — v0.4 vs v0.5 vs v0.7 on held-out IMG_3651\n"
...
def crop_from_n_models(all_kpts_per_model, window, src_w, src_h, pad=80):
    ...
    for kpts_list in all_kpts_per_model:
        for i in range(a, b):
            ...
            if not valid.any():
                continue
            xs.extend(arr[:, :, 0][valid].tolist())
            ys.extend(arr[:, :, 1][valid].tolist())
    if not xs:
...
if __name__ == "__main__":
    ...
    _, v07_kpts, v07_all = predict_clip(V07, "v0.7")

    cap = cv2.VideoCapture(str(CLIP))
    fps = cap.get(cv2.CAP_PROP_FPS) or 30.0
    cap.release()

    chart_path = ART / f"ear_angle_lines-v04v05v07-{CLIP.stem}.png"
    make_ear_angle_chart(
        {"v0.4": v04_kpts, "v0.5": v05_kpts, "v0.7": v07_kpts},
        fps, chart_path,
    )
    print(f"\nEar angle chart -> {chart_path}")

    sxs_path = ART / f"v0.4-vs-v0.5-vs-v0.7-{CLIP.stem}.mp4"
    make_sxs(
        ["v0.4", "v0.5", "v0.7"],
        [V04_BGR, V05_BGR, V07_BGR],
        [v04_all, v05_all, v07_all],
        sxs_path,
    )
    print(f"SxS video -> {sxs_path}")

    stills_path = ART / f"v0.4-vs-v0.5-vs-v0.7-{CLIP.stem}.png"
    make_stills(
        ["v0.4", "v0.5", "v0.7"],
        [V04_BGR, V05_BGR, V07_BGR],
        [v04_kpts, v05_kpts, v07_kpts],
        [v04_all, v05_all, v07_all],
        stills_path,
    )
    ...
    a, b = WINDOW
    report = {}
    for label, kpts in [("v0.4", v04_kpts), ("v0.5", v05_kpts), ("v0.7", v07_kpts)]:
    ...
    report_path = ART / f"bench_report-{CLIP.stem}-v04v05v07.json"
