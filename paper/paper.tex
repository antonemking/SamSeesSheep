...
\begin{figure}[t]
  ...
  \caption{SamSeesSheep pipeline output on the held-out \texttt{Test\_Clip\_Morning}
  clip: simultaneous ear-angle monitoring of six animals. \emph{Left:} the
...
\begin{enumerate}
  ...
  two independent held-out clips (\texttt{IMG\_3651} and
  \texttt{Test\_Clip\_Morning}) spanning different flock arrangements, lighting,
  and model versions.
...
\begin{table}[t]
  ...
  on the \texttt{IMG\_3651} held-out clip.}
  \label{tab:dataset}
  \begin{tabular}{lcccc}
    \toprule
...
Two clips were held out from all labeling and training: \texttt{IMG\_3651} (v0.2--v0.7
benchmarks) and \texttt{Test\_Clip\_Morning} (calibrated for v0.7).
...
\paragraph{Test\_Clip\_Morning.}
$1920\times1080$, 986 frames, ${\sim}30$\,fps, 61\,MB, filmed on the same pasture
with the same flock under morning lighting distinct from IMG\_3651's afternoon
...
IMG\_3651 window. v0.7 detection rate within the window is 148/150 (98.7\%).
...
\texttt{sheep} boxes on 323/933 frames (35\%) for IMG\_3651 and 118/986 frames
(12\%) for Test\_Clip\_Morning, but \emph{zero keypoints} on either clip; ear
angle is unmeasurable without a keypoint head. (On Test\_Clip\_Morning the model
fires \emph{some} box on every frame, but the overwhelming majority are
misclassifications --- predominantly COCO ``dog'' --- so the sheep-class rate,
not the any-class rate, is the honest detection figure.)

\section{Results}

\subsection{Detection and Keypoint Stability (IMG\_3651)}
Table~\ref{tab:headline} reports the primary results on IMG\_3651 for v0.2--v0.4
plus the stock baseline.

\begin{table}[t]
  \centering
  \caption{IMG\_3651 held-out results. Stock-YOLO detection is the sheep-class
  rate (Section~\ref{sec:stock}); it produces no keypoints.}
  \label{tab:headline}
  \begin{tabular}{lcccc}
    \toprule
...
\subsection{Per-Keypoint Residual $\sigma$ (IMG\_3651)}
...
\begin{table}[t]
  \centering
  \caption{Per-keypoint residual $\sigma$ (px) on IMG\_3651.}
  \label{tab:perkpt}
  \begin{tabular}{lcccc}
    \toprule
...
\subsection{Ear-Angle Stability (IMG\_3651)}
Table~\ref{tab:earangle} reports the derived ear-angle residual $\sigma$. This is
the welfare-relevant scalar: a stationary sheep should produce a flat ear-angle
...
\begin{table}[t]
  \centering
  \caption{Ear-angle residual $\sigma$ (degrees) on IMG\_3651.}
  \label{tab:earangle}
  \begin{tabular}{lcc}
    \toprule
...
\begin{figure}[t]
  ...
  \caption{Ear angle over the motionless window on IMG\_3651 for v0.2/v0.3/v0.4.
  Flatter traces indicate a usable signal; noisier traces indicate jitter
  masking it. Legend $\sigma$ values are the residual standard deviations of
  Table~\ref{tab:earangle}. Shaded bands are the SPFES reference zones
  (green $\geq 30\dgr$, amber $-10\dgr$ to $30\dgr$, red $\leq -10\dgr$); they are
  literature reference bands, not diagnostic cutoffs.}
  \label{fig:earangle}
\end{figure}
...
benchmarked on IMG\_3651 (Table~\ref{tab:progression}). The monotonic
...
mixed regression --- right ear $3.55\dgr$ (best recorded on IMG\_3651) but left
...
$3.70\dgr$ (right $4.46\dgr$). Across all versions on IMG\_3651, $\sigavg$
...
\begin{table}[t]
  \centering
  \caption{Full progression on IMG\_3651. Improvement is monotonic only through
...
\begin{table}[t]
  ...
  \begin{tabular}{llcc}
    ...
    \midrule
    IMG\_3651 & v0.2 & 6.71 [5.56, 8.04] & 6.07 [4.65, 7.60] \\
    IMG\_3651 & v0.4 & 4.06 [2.97, 4.99] & 4.09 [2.83, 5.26] \\
    IMG\_3651 & v0.7 & 3.70 [2.96, 4.10] & 4.46 [3.08, 5.64] \\
    Test\_Clip\_Morning & v0.2 & 5.37 [1.31, 8.82] & 2.24 [0.95, 3.09] \\
    Test\_Clip\_Morning & v0.4 & 2.50 [1.15, 3.35] & 2.72 [1.22, 3.69] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Second Held-Out Clip: Test\_Clip\_Morning}
\label{sec:morning}
The second clip provides a cross-clip validation point, testing whether the
training-scale monotonicity observed on IMG\_3651 (v0.2$\rightarrow$v0.4)
...
pattern as IMG\_3651. Raw $\sigma$ varies 41--58\,px, driven by lateral head
drift rather than model noise.

\begin{table}[t]
  \centering
  \caption{Test\_Clip\_Morning held-out results. Stock-YOLO detection is the
  sheep-class rate (Section~\ref{sec:stock}); the any-class rate is 100\% but is
  dominated by non-sheep misclassifications.}
...
differs from IMG\_3651: v0.2's largest variance is on the right ear (base
...
${\sim}1.4$--$1.8\%$ of head width, versus $3.3$--$4.2\%$ on IMG\_3651's
234-px-wide head; the larger apparent head (${\sim}1.35\times$) partly but not
fully explains the lower pixel $\sigma$.

\begin{table}[t]
  \centering
  \caption{Per-keypoint residual $\sigma$ (px) on Test\_Clip\_Morning.}
...
IMG\_3651 the left/right $\sigma$ were roughly symmetric; here v0.2 shows a
...
\begin{table}[t]
  \centering
  \caption{Ear-angle residual $\sigma$ (degrees) on Test\_Clip\_Morning.}
...
achieves $\sigavg = 4.08\dgr$ on IMG\_3651 and $2.61\dgr$ on Test\_Clip\_Morning
--- a 36\% difference attributable to the clip, not the model. Three factors
likely contribute: the larger apparent head on Test\_Clip\_Morning
...
version improvement: \textbf{the $2.84\dgr$ reached by v0.7 on Test\_Clip\_Morning
...
\begin{table}[t]
  \centering
  \caption{Cross-clip comparison of ear-angle $\sigavg$. The IMG\_3651 vs.\
  Test\_Clip\_Morning gap is a clip-difficulty effect, not a model improvement.}
  \label{tab:crossclip}
  \begin{tabular}{lccc}
    \toprule
...
The headline result --- ${\sim}4.08\dgr$ ear-angle $\sigavg$ on IMG\_3651 (v0.4)
and ${\sim}2.84\dgr$ on Test\_Clip\_Morning (v0.7) --- establishes a noise floor
...
parameters, ${\sim}10$\,MB) that places five keypoints on every detected sheep
head and achieves ear-angle residual $\sigavg$ of $4.08\dgr$ (v0.4 on IMG\_3651)
and $2.84\dgr$ (v0.7 on Test\_Clip\_Morning) --- a metric literally unmeasurable
...
The code, dataset exports, trained weights, benchmark scripts, and full
documentation are available under the MIT License at
\url{https://github.com/antonemking/SamSeesSheep}. Every $\sigma$, detection
rate, and per-keypoint number in this paper is reproducible from saved JSON
artifacts under \texttt{sheep-yolo/artifacts/}, checkable in one command via
\texttt{sheep-yolo/scripts/verify\_paper\_claims.py} (36/36 checks pass against
the artifacts). The per-frame model predictions used for every benchmark are
cached in \texttt{sheep-yolo/artifacts/\_cache/}, and the held-out benchmark
JSONs can be regenerated from them
(\texttt{gen\_bench\_Test\_Clip\_Morning.py}; the archived
\texttt{bench\_v02\_v03.py} / \texttt{bench\_v04\_v05\_v0\{6,7\}.py}). The
bootstrap confidence intervals of Table~\ref{tab:ci} are in
\texttt{bench\_bootstrap\_ci.json}. The hero video of Figure~\ref{fig:hero} is
\texttt{sheep-yolo/artifacts/synced-lanes-6ewes-pro-Test\_Clip\_Morning.mp4}.

Two figures are reported from sources outside this repository and are not
checkable by the verification script: the validation mAP50-95 values
(Table~\ref{tab:dataset}) come from the Ultralytics training logs produced on the
cloud GPU, and the clip-similarity screening used to select held-out clips was
performed at capture time (the held-out clips were additionally never pushed to
the labeler, enforced by per-file checksum dedup). Raw video and the full
reviewed-keypoint dataset are retained off-repository to limit size and are
