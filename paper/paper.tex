...
size and published on the Hugging Face Hub at \url{<HF-model-repo-URL>}, with a
model card and a persistent DOI (\texttt{<HF DOI placeholder>}) pinned to a fixed
revision. The per-frame model-prediction caches
(\texttt{sheep-yolo/artifacts/\_cache/*.pkl}, ${\sim}3$\,MB total) ship as an asset
on a tagged GitHub release of the code repository. With the caches, every
