...
\affil[1]{Independent Researcher, Middletown, Delaware, USA \\ \texttt{toneking972@gmail.com}}
\date{}
...
edge-runnable keypoint detector (YOLO-pose, 2.5\,M parameters, ${\sim}6$\,MB)
...
\begin{figure}[t]
  ...
  ear-angle extraction, running on an edge GPU with a ${\sim}6$\,MB model.}
  \label{fig:hero}
\end{figure}
...
from ambient pasture video with a quantified, reproducible stability benchmark. Building one
...
on a cloud GPU; and the resulting weights (${\sim}6$\,MB) run inference on a
single consumer GPU. That small footprint is deliberate: low-power edge
deployment is the design target (a roadmap goal), and the benchmarks here use a
6\,GB consumer card as a stand-in. The pipeline produces per-keypoint residual
...
\begin{enumerate}
  ...
  parameters, ${\sim}6$\,MB) that places five keypoints on every detected sheep
  ...
  confidence intervals, a window-selection sensitivity sweep, and a stock-YOLO
  baseline. The protocol is applied to
...
\begin{figure}[t]
  ...
  \phantom{xxxxxxxxxxxxxx}$\rightarrow$ Edge inference + $\sigma$ benchmark (consumer 6 GB GPU)
  \end{minipage}}
...
training completes in ${\sim}6$ minutes and \texttt{best.pt} is ${\sim}6$\,MB.

\subsection{Edge Inference and Benchmark Protocol}
\label{sec:bench}
For these benchmarks, inference runs on a single local consumer GPU (GTX~1660~Ti,
6\,GB) --- a stand-in for the low-power edge devices that are the intended
deployment target, which the ${\sim}6$\,MB model is sized for. The protocol, implemented in
...
floor is ${\sim}10\%$ of that span --- a scale comparison only, not a claim that
the instrument is calibrated for SPFES band assignment. The stock baseline
...
\subsection{Window-Selection Sensitivity}
\label{sec:winsens}
Each headline $\sigma$ is computed on one hand-picked ``calmest ${\sim}5$\,s''
window, which raises a fair concern: selecting the window on the variable being
minimized (head motion) could bias the reported noise floor downward. To bound
this effect I slid the same fixed-length window across the target sheep's entire
detected span (step 2 frames; windows retained when $\geq 60$ frames carry a
computable ear angle) and recomputed $\sigavg$ and the within-window head motion
(raw positional $\sigma$) for every window. The procedure is implemented in
\texttt{sheep-yolo/scripts/window\_sensitivity.py} and reproduces each paper
window's committed $\sigavg$ and raw $\sigma$ exactly. Table~\ref{tab:winsens}
and Figure~\ref{fig:winsens} report the resulting distributions.

\begin{table}[t]
  \centering
  \caption{Window-selection sensitivity. The same fixed-length window is slid
  across each target's detected span; $\sigavg$ is the mean left/right ear-angle
  residual $\sigma$ per window. ``Pctile'' is the fraction of windows whose
  $\sigavg$ is at or below the hand-picked paper window's --- low means the paper
  window is among the calmer (lower-$\sigma$) ones.}
  \label{tab:winsens}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lcccccc}
    \toprule
    Run & Windows & Paper $\sigavg$ (\dgr) & Pctile & Median $\sigavg$ (\dgr) & Max $\sigavg$ (\dgr) & corr(motion, $\sigma$) \\
    \midrule
    HO-1 v0.4 & 200 & 4.07 & 18th & 4.55 & 5.69 & 0.56 \\
    HO-1 v0.7 & 205 & 4.08 & 43rd & 4.43 & 8.43 & 0.47 \\
    HO-2 v0.4 & 324 & 2.61 & 75th & 2.37 & 6.32 & 0.29 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{window_sensitivity.png}
  \caption{Ear-angle residual $\sigavg$ for every $5$\,s window slid across the
  target's detected span, versus within-window head motion. Red star: the
  hand-picked paper window; dashed line: the median window. Across all windows
  and both clips $\sigavg$ stays a small fraction of the SPFES $40\dgr$ band and
  rises only modestly with head motion --- the reported noise floor is not an
  artifact of selecting the stillest segment.}
  \label{fig:winsens}
\end{figure}

The hand-picked window does sit low in the HO-1 v0.4 distribution (18th
percentile), confirming the selection mechanism is real --- but its magnitude is
small. The median window scores $4.55\dgr$ against the reported $4.07\dgr$, and
\emph{even the noisiest} $5$\,s window of that sheep reaches only $5.69\dgr$
(${\sim}14\%$ of the SPFES $40\dgr$ span). For v0.7 the paper window is
mid-distribution (43rd percentile), and on HO-2 it is above the median (75th
percentile) --- a higher-motion window than typical, so the reported $2.61\dgr$
is, if anything, conservative. Within-window motion correlates with $\sigavg$
only moderately ($r = 0.29$--$0.56$), so motion explains part but not most of the
window-to-window variance. The noise floor is therefore not an artifact of
selecting the stillest segment: across every window of both targets $\sigavg$
stays between ${\sim}1.3\dgr$ and ${\sim}8.4\dgr$, a small fraction of the SPFES
band in all cases.
...
output on this clip. By design, HO-2 carries the v0.2--v0.4 trend-reproduction
set plus v0.7 as the then-current model; v0.5 and v0.6 were benchmarked only on
HO-1 (the full ladder lives in Table~\ref{tab:progression}). The v0.7 run on HO-2
recorded the aggregate ear-angle $\sigma$ (Table~\ref{tab:morningearangle}) rather
than a per-keypoint breakdown, so the per-keypoint Table~\ref{tab:morningperkpt}
lists v0.2--v0.4.
...
\begin{table}[t]
  \centering
  \caption{Per-keypoint residual $\sigma$ (px) on HO-2. Per-keypoint data exist
  for v0.2--v0.4 on HO-2; see Section~\ref{sec:morning} for the version coverage.}
...
training progression. Consequently the reported $\sigma$ quantify
\emph{precision} (frame-to-frame self-consistency), not accuracy: with no
independent ground-truth landmark, a fixed systematic error in keypoint
placement would be invisible to this benchmark, as would any slow bias absorbed
by the 7-frame rolling median.
...
\paragraph{Window selection.} The benchmark window is the calmest available
${\sim}5$\,s segment, which could bias the noise floor downward. The sensitivity
sweep of Section~\ref{sec:winsens} bounds this effect: it is real but small (the
HO-1 v0.4 window is at the 18th percentile, ${\sim}0.5\dgr$ below the median
window, and even the noisiest window of that target is $5.69\dgr$), and on HO-2
the chosen window is actually above the median. The reported floors are
best-to-typical, not unrepresentative outliers.

\paragraph{Two held-out clips, same flock and same animals.} The protocol uses
two held-out clips spanning different lighting and arrangements, but both are
from the same flock, pasture, and camera --- and both show the same five
individuals. ``Held-out'' therefore demonstrates generalization to unseen poses,
scenes, and lighting on \emph{known} animals, not to unseen animals;
animal-level held-out evaluation requires a second flock. A multi-clip ablation
(3--5 clips from different sessions and weather) would strengthen the
...
\paragraph{No student-vs-teacher comparison.} The trained detector is not
benchmarked against the SAM~3 teacher labels it distilled from, so whether the
student matches, exceeds, or trails the teacher on held-out frames is unmeasured
(SAM~3 was never run on the held-out clips, by design). A student-vs-teacher
agreement study on held-out frames is the most direct next ablation.
...
The author's pipeline code, benchmark scripts, machine-readable benchmark
artifacts, paper source, and documentation are available under the MIT License at
\url{https://github.com/antonemking/SamSeesSheep}; the trained YOLO-pose weights
carry a separate license inherited from Ultralytics (see Companion artifacts
below). Reproducibility is layered, so
...
and re-validates each value against the paper (59/59 checks pass on a fresh
...
pinned to a fixed revision. Because they are trained with Ultralytics YOLO
(AGPL-3.0), the weights are released under AGPL-3.0 rather than MIT; only the
author's own repository code is MIT-licensed. The per-frame model-prediction caches
...
off-repository; and the held-out clips were screened by pixel normalized cross-correlation (NCC) of
the first frame against every training video --- HO-1 scored NCC ${<}0.23$ ---
and were held in a separate \texttt{test-clips/} directory, never pushed to the
labeler (enforced by per-file checksum dedup). The training-video frames needed
to recompute these NCC scores are themselves off-repository, so this screening
remains an author attestation the public artifacts cannot independently
re-derive.
