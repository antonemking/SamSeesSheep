...
\paragraph{Companion archive (regenerate from cached predictions).} To keep the
git tree small, the per-frame model-prediction caches
(\texttt{sheep-yolo/artifacts/\_cache/*.pkl}, ${\sim}3$\,MB total) and the trained
per-version weights are \emph{not} tracked in git; they are published as an asset
on a tagged GitHub release of the same repository [persistent archive DOI to be
minted on Zenodo: \texttt{<DOI placeholder>}]. With the caches, every committed
benchmark JSON can be regenerated from scratch
(\texttt{gen\_bench\_Test\_Clip\_Morning.py} for HO-2; the archived
\texttt{bench\_v02\_v03.py} / \texttt{bench\_v04\_v05\_v0\{6,7\}.py} for HO-1), and
the ear-angle chart of Figure~\ref{fig:earangle} can be regenerated with
\texttt{gen\_ear\_angle\_chart.py}. The hero still of Figure~\ref{fig:hero} is a
frame from \texttt{synced-lanes-6ewes-pro-Test\_Clip\_Morning.mp4}, also in the
archive.
